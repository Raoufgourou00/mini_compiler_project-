...
;
A:  idf croch_ouv EXP croch_fer
    {
        if(!Entite_Non_Declare($1)) {      
            if(!Est_Une_Constante($1)) {
                if(Est_Un_Tableau($1)) {
                    
                    nom_idf = Allouer_Char(strlen($1));
                    strcpy(nom_idf, $1);

                    Empiler_Type(0);
                    Traitement_Types();
                    Incompatibilite_Types_Index($1);
...
EXP:  EXP plus EXP          
    ...
    | idf croch_ouv EXP croch_fer
    {
        ...
        char temp[20];
        Depiler(temp);
        Affectation_QUAD("", Case_Tab($1,temp));
    }             
    | entier                
    {   
        Empiler_Type(0);
